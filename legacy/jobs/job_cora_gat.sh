...
python3 -u "${SCRIPT}"   --dataset "${DATASET_ARG}"   --k-values "${K_VALUES}"   --seeds "${SEEDS}"   --methods "${METHODS}"   --output-dir "${RESULT_DIR}"   --plots-dir "${PLOTS_DIR}"   --hidden-channels 8   --heads 8   --dropout 0.6   --learning-rate 0.005   --weight-decay 5e-4   --epochs 300   --early-stopping   --save-report   2>&1 | tee "${LOG_FILE}"
