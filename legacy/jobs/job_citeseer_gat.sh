#!/bin/bash
#SBATCH --job-name=citeseer_gat_cls
#SBATCH --chdir=/scratch/rdongmo/IDP-LS/k-Anonymization
#SBATCH --output=logs/citeseer_gat_%j.out
#SBATCH --error=logs/citeseer_gat_%j.err
...
RESULT_DIR="resultat/citeseer_gat_classification_k_sweep"
PLOTS_DIR="plots_citeseer_gat"
LOG_FILE="logs/citeseer_gat_${JOB_ID}_${RUN_TIMESTAMP}.log"
...
REPORT_PDF="${RESULT_DIR}/results_citeseer_gat_classification_report.pdf"
REPORT_HTML="${RESULT_DIR}/results_citeseer_gat_classification_report.html"
DETAILED_CSV="${RESULT_DIR}/results_citeseer_gat_classification.csv"
SUMMARY_CSV="${RESULT_DIR}/results_citeseer_gat_classification_summary.csv"
EXPECTED_PLOTS=(
...
echo "========================================================"
echo "JOB Citeseer --- GAT Classification"
echo "Job ID   : ${JOB_ID}"
echo "Noeud    : $(hostname)"
echo "Demarre  : $(date)"
...
echo "[CHECK] Dependances Python critiques (GAT) :"
python3 - <<'PY'
import importlib
import sys
...
try:
    ...
    missing.append(f"torch_geometric.nn.GATConv: {exc}")
...
print("[OK] Dependances GAT disponibles.")
PY
