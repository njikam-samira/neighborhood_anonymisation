...
echo "========================================================"
echo "JOB Cora --- Node2Vec Classification"
echo "Job ID   : ${JOB_ID}"
echo "Noeud    : $(hostname)"
echo "Demarre  : $(date)"
...
python3 -u "${SCRIPT}"   --dataset "${DATASET_ARG}"   --k-values "${K_VALUES}"   --seeds "${SEEDS}"   --methods "${METHODS}"   --output-dir "${RESULT_DIR}"   --plots-dir "${PLOTS_DIR}"   --embedding-dim 128   --walk-length 20   --context-size 10   --walks-per-node 10   --p 1.0   --q 1.0   --epochs 100   --save-report   2>&1 | tee "${LOG_FILE}"
