...
def calculate_cluster_cost(cluster: List[Dict[int, int]]) -> Tuple[float, float, int, int]:
    ...
    cost_mean = sum(abs(degree - mean_degree) for degree in degrees)
    cost_median = sum(abs(degree - median_degree) for degree in degrees)
    return cost_mean, cost_median, mean_degree, median_degree

def cluster_formation_MAE(P: List[Dict[int, int]], k: int) -> Tuple[List[List[Dict[int, int]]], List[int], List[Dict[int, int]]]:
    """
